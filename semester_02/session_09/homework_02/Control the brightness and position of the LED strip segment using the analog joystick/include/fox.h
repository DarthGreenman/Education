...
namespace fox
{
	...
	{
		template <typename... T> expand_parameter_package(T...) {}
	};
	...
	fox::pair<T, U> make_pair(T &&first, U &&second) { return fox::pair<T, U>{first, second}; }

	template <typename T, size_t N> size_t find(const T(&elems)[N], T elem)
	{
		for (size_t n{}; n < N; ++n)
			if (elems[n] == elem)
				return n;
