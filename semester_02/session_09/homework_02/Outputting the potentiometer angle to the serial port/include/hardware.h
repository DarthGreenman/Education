...
inline electronic_component<Number_of_channels>::electronic_component(
    const wokwi::channel_connect_params (&pins)[Number_of_channels])
{
...
template <uint8_t Number_of_channels>
inline const wokwi::channel_connect_params &electronic_component<Number_of_channels>::get_params(
...
{
    assert((signal_value == digital_signal_values::low || signal_value == digital_signal_values::high) &&
...
inline void electronic_component<Number_of_channels>::write(uint8_t channel_number, uint16_t signal_value)
{
    assert(signal_value >= static_cast<uint32_t>(boundary_values_of_analog_signal::low) &&
           signal_value <= static_cast<uint32_t>(boundary_values_of_analog_signal::high) &&
           "Incorrect analog signal value.");
    const auto &ch = get_params(channel_number);
...
template <uint8_t Number_of_channels>
inline uint16_t electronic_component<Number_of_channels>::read(uint8_t channel_number) const
{
    const auto &ch = get_params(channel_number);
...
template <uint8_t Number_of_channels>
inline bool electronic_component<Number_of_channels>::is_valid_pin_params(uint8_t pin, wokwi::signal_type signal_type,
                                                                          wokwi::signal_direction direction)
{
    switch (signal_type)
    {
