...
namespace wokwi
{
		...
		for (uint8_t channel_number{}; channel_number < Number_of_channels; ++channel_number)
		{
			if (_com == common_pin::anode)
			{
				channels[channel_number].type == wokwi::signal_type::analog
					...
					: basic::write(channel_number, wokwi::digital_signal_values::high);
			}
		}
	}
	...
	inline void light_emitting_diode<Number_of_channels>::increase_brightness(wokwi::rgb_colour channel)
	{
		...
		const uint8_t signal_value = _com == common_pin::anode
										 ? wokwi::digital_signal_values::low
										 : wokwi::digital_signal_values::high;
	...
	template <>
	class light_emitting_diode<1u> : public wokwi::electronic_component<1u>
	{
	public:
