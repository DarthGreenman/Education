...
namespace wokwi
{
template <uint8_t Number_of_channels> class light_emitting_diode : public wokwi::electronic_component<Number_of_channels>
{
...
template <uint8_t Number_of_channels> inline void light_emitting_diode<Number_of_channels>::increase_brightness(wokwi::rgb_colour channel)
{
...
template <uint8_t Number_of_channels> inline void light_emitting_diode<Number_of_channels>::decrease_brightness(wokwi::rgb_colour channel)
{
	const auto channel_number = helper::find(_channels, channel);
...
template <uint8_t Number_of_channels>
inline void light_emitting_diode<Number_of_channels>::change_brightness(wokwi::rgb_colour channel, uint16_t signal_value)
{
	const auto channel_number = helper::find(_channels, channel);
	basic::write(channel_number, signal_value);
}
...
public:
	...
	light_emitting_diode() = default;
	explicit light_emitting_diode(uint8_t pin, wokwi::signal_type type, wokwi::signal_direction direction);
	light_emitting_diode(const light_emitting_diode&) = default;
	light_emitting_diode(light_emitting_diode&&) = default;
	~light_emitting_diode() = default;
...
inline light_emitting_diode<1u>::light_emitting_diode(uint8_t pin, wokwi::signal_type type, wokwi::signal_direction direction)
	: wokwi::electronic_component<1u>{pin, type, direction}
{
}
...
{
	!basic::read(_channel_number) ? change_brightness(signal_value)
