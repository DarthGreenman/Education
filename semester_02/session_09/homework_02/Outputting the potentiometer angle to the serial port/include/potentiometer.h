...
template <typename Low_pass_filter> class potentiometer : public wokwi::electronic_component<1u>
{
  public:
    ...
    explicit potentiometer(uint8_t pin, wokwi::signal_type type, wokwi::signal_direction direction,
                           helper::couple<int, int> boundaries_of_scale =
                               {static_cast<uint32_t>(wokwi::boundary_values_of_analog_signal::low),
                                static_cast<uint32_t>(wokwi::boundary_values_of_analog_signal::high)},
...
inline potentiometer<Low_pass_filter>::potentiometer(uint8_t pin, wokwi::signal_type type,
                                                     wokwi::signal_direction direction,
                                                     helper::couple<int, int> boundaries_of_scale,
                                                     Low_pass_filter low_pass_filter)
    : wokwi::electronic_component<1u>{pin, type, direction}, _handler_table{}, _signal_value{},
      _boundaries_of_scale{boundaries_of_scale}
{
