...
public:
     ...
     */
    explicit task(const_pointer_to_function_type function, const_pointer_to_object_type object, uint32_t polling_time_interval,
     ...
     * @param time_before_start
     */
    explicit task(const_pointer_to_function_type function, T&& object, uint32_t polling_time_interval, uint32_t time_before_start = 0ul)
        : _object { object }
        , _cptr_object { &_object }
        , _cptr_func { function }
        , _timeslot { polling_time_interval, millis() + time_before_start, millis() }
        , _we_work { }
    {
