...
public:
    ...
    {
        is_signal_analog<Ts>::value ? analogWrite(_pin_number, signal_value)
    ...
    uint16_t read() const
    {
        return is_signal_analog<Ts>::value ? analogRead(_pin_number)
                                           : digitalRead(_pin_number);
    }
